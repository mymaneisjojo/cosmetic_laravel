...
<form role="form" method="post" action="{{route('color.update', $model->id)}}"> 
   
    <div class="card-body">
    @csrf @method('PUT')
        <div class="form-group">
            <label for="exampleInputEmail1">Name</label>
            <input name="name" class="form-control" id="exampleInputEmail1" placeholder="Enter name" value="{{$model->name}}">
            @error('name')
        ...
        <div class="form-group">
            ...
            <select name="parent_id" required="required" id="" class="custom-select">
                @foreach($colors as $colo)
                <?php $selected = $model->parent_id == $colo->id ? 'selected' : '';?>
